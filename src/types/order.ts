...
export function getWarrantyStatus(order: ServiceOrder): { status: WarrantyStatus; daysRemaining: number } {
  if (!order.warrantyEndDate || !order.warrantyStartDate) {
    return { status: "none", daysRemaining: 0 };
  }

  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const endDate = new Date(order.warrantyEndDate);
