        ...
        <ToastContext.Provider value={value}>
            ...
            <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 max-w-sm w-full pointer-events-none">
                {toasts.map((t) => (
                    <div
                        key={t.id}
                        className={`pointer-events-auto flex items-start w-full max-w-sm overflow-hidden bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-100 ring-1 ring-black/5 transform transition-all duration-300 ease-in-out translate-y-0 opacity-100 p-4`}
                        role="alert"
                    >
                        ...
                        <div className="w-full  pt-0.5">
                            {t.title && <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">{t.title}</p>}
