...
import { Save, ArrowLeft, Printer, CheckCircle, Eye, MessageCircle, PlusCircle, FileDown, History, AlertCircle } from "lucide-react";
...
export default function NuevaOrdenPage() {
  ...
  const searchHistoryBySerial = async (serial: string) => {
    ...
    try {
      const res = await fetch(`/api/orders/search?q=${encodeURIComponent(serial)}&type=serial`);
      if (res.ok) {
        const data = await res.json();
        setSerialHistory(Array.isArray(data) ? data : []);
      } else {
        setSerialHistory([]);
      }
    } catch {
  ...
  const handlePrintCreated = async () => {
    ...
    const portalLink = `${window.location.host}/orden/${o.orderNumber}`;
...
<div style="text-align:center;margin-top:15px">
  <p style="font-size:10px;margin:0 0 5px 0">Escanee para ver estado en vivo:</p>
  <img src="${qrUrl}" width="100" height="100" style="display:block;margin:0 auto" />
  <p style="font-size:8px;margin:5px 0 0 0;color:#666">${portalLink}</p>
</div>
              ...
              {serialHistory.length > 0 && (
                <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <p className="text-xs font-bold text-amber-800 flex items-center gap-1 mb-2">
                    ...
                    Antecedentes encontrados ({serialHistory.length})
                  </p>
                  <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
                    {serialHistory.map((h, i) => (
                      <div key={i} className="text-[10px] bg-white/50 p-2 rounded border border-amber-100 flex justify-between items-start gap-1">
                        <div>
                          <p className="font-semibold text-gray-900">{h.orderNumber}</p>
                          <p className="text-gray-500 line-clamp-2">{h.problemDescription}</p>
                          <p className="text-[8px] text-gray-400 mt-0.5">{new Date(h.createdAt).toLocaleDateString()}</p>
                        </div>
                        <span className={`status-badge text-[8px] px-1 py-0 shadow-none`}>
                          {h.status}
                        </span>
                      </div>
                    ))}
