...
import { ArrowLeft, Save, Copy, Check, CheckCircle, Printer, Plus, Trash2, Clock, MessageSquare, MessageCircle, Wrench, FileText, Send, ThumbsUp, ThumbsDown, ChevronDown, Eye, Lock, Wallet, Smartphone, Lightbulb, AlertTriangle } from "lucide-react";
...
export default function OrderDetailPage() {
  ...
  const sendBudget = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const phone = order.customerPhone.replace(/\D/g, "");
        const fullPhone = phone.startsWith(cc) ? phone : `${cc}${phone}`;
        const portalLink = `${window.location.origin}/orden/${order.orderNumber}`;
        const defaultTemplate = `Hola {nombre}, hemos preparado un presupuesto para tu equipo {equipo}. Revisa los detalles y apruébalo aquí:\n\n👉 {link}\n\nOrden: {orden}`;
        const template = s.whatsappTemplateBudget?.trim() || defaultTemplate;
        const msg = template
          .replace(/\{nombre\}/g, order.customerName)
          .replace(/\{equipo\}/g, `${order.deviceBrand} ${order.deviceType}`)
  ...
  const handlePrint = async () => {
    ...
    const portalLink = `${window.location.host}/orden/${order.orderNumber}`;
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(portalLink)}`;
...
.terms{margin-top:16px;padding-top:12px;border-top:1px dashed #ccc;font-size:9px;color:#999;line-height:1.4}
.qr-container{text-align:center;margin-top:15px;display:flex;flex-direction:column;align-items:center;gap:4px}
.qr-container img{display:block;margin:0 auto}</style></head><body>
...
<div class="qr-container">
  <p style="font-size:10px;margin:0">Escanee para ver estado en vivo:</p>
  <img src="${qrUrl}" width="100" height="100" />
  <p style="font-size:8px;margin:0;color:#666">${portalLink}</p>
</div>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <Link href="/admin/ordenes" aria-label="Volver a órdenes" className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={copyOrderNumber} aria-label="Copiar número de orden" className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors" title="Copiar número">
                {copied ? <Check className="h-4 w-4 text-green-500" /> : <Copy className="h-4 w-4 text-gray-400 dark:text-gray-500" />}
              </button>
            </div>
      ...
      {/* Messages */}
      {error && <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg text-sm">{error}</div>}
      {success && <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg text-sm">{success}</div>}

      {/* Status Selector */}
      <div className="card mb-6">
        <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Estado de la Orden</h3>
          ...
          {ALL_STATUSES.map((status) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2.5 min-h-[44px] rounded-lg text-sm font-medium transition-all duration-200 border-2 flex items-center gap-1.5 ${isActive
                  ...
                  : budgetBlocked
                    ...
                    : "bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500 active:bg-gray-50 dark:active:bg-gray-700"
                  }`}
              >
                {config.label}
                {budgetBlocked && <Lock className="h-3.5 w-3.5 ml-0.5" />}
              </button>
            );
          })}
        </div>
        {order.budgetStatus === "pending" && (
          <p className="mt-3 text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 rounded-lg px-3 py-2 flex items-center gap-1.5">
            <AlertTriangle className="h-3.5 w-3.5 shrink-0" /> No puedes avanzar a reparación hasta que el cliente apruebe el presupuesto.
          </p>
        )}
        {order.budgetStatus === "rejected" && (
          <p className="mt-3 text-xs text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/30 rounded-lg px-3 py-2 flex items-center gap-1.5">
            <AlertTriangle className="h-3.5 w-3.5 shrink-0" /> El cliente rechazó el presupuesto. Re-envía uno nuevo para poder continuar.
          </p>
        )}
      </div>

      {/* Client Portal QR Mini-Card */}
      <div className="card mb-6 border-primary-100 bg-primary-50/30 flex items-center gap-4">
        <div className="p-1.5 bg-white border border-gray-100 rounded-xl shrink-0 shadow-sm">
          <img
            src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(`${typeof window !== 'undefined' ? window.location.host : ''}/orden/${order.orderNumber}`)}`}
            alt="QR Portal"
            width={100}
            height={100}
        ...
        <div>
          <h4 className="text-xs font-bold text-primary-900 uppercase tracking-wider flex items-center gap-1.5">
            ...
            Portal del Cliente en Vivo
          </h4>
          <p className="text-[11px] text-primary-700/70 mt-1 leading-relaxed max-w-xs">
            Muestra este QR al cliente para que vea fotos, apruebe presupuestos y siga el estado de su equipo sin registrarse.
          </p>
          <div className="flex gap-3 mt-3">
            <button
              onClick={() => {
                const link = `${window.location.origin}/orden/${order.orderNumber}`;
                navigator.clipboard.writeText(link);
                setCopied(true);
                setTimeout(() => setCopied(false), 2000);
              }}
              className="text-xs font-semibold text-primary-600 hover:text-primary-700 flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-lg border border-primary-100 shadow-sm"
            >
            ...
            <Link
              ...
              target="_blank"
              className="text-xs font-semibold text-gray-600 hover:text-gray-700 flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-lg border border-gray-200 shadow-sm"
            >
      ...
      <div className="space-y-6">
        ...
        <div className="card">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Datos del Cliente</h3>
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Datos del Equipo</h3>
          <div className="space-y-4">
              ...
              <div className="p-5 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border-2 border-green-200 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                    <Wallet className="h-4 w-4 text-green-600" /> Presupuesto Total
                  </span>
            ...
            {(!order.budgetStatus || order.budgetStatus === "none" || order.budgetStatus === "rejected") && (order.estimatedCost > 0 || selectedServices.length > 0) && (
              <div className="p-5 bg-gradient-to-br from-blue-50 to-primary-50 rounded-xl border-2 border-primary-300 shadow-sm">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400 mb-4 leading-relaxed flex items-start gap-1.5">
                  <Smartphone className="h-3.5 w-3.5 shrink-0 mt-0.5" /> El cliente recibirá un WhatsApp con el link al portal donde podrá aprobar o rechazar el presupuesto.
                </p>
                <textarea
                  value={budgetNote}
            ...
            {(!order.budgetStatus || order.budgetStatus === "none") && selectedServices.length === 0 && (order.estimatedCost || 0) === 0 && (
              <div className="p-4 bg-amber-50 border-2 border-dashed border-amber-300 rounded-xl text-center">
                <p className="text-sm text-amber-800 dark:text-amber-300 font-medium mb-1 flex items-center justify-center gap-1.5">
                  <Lightbulb className="h-4 w-4 shrink-0" /> Agrega servicios o define un costo para enviar el presupuesto
                </p>
            ...
            {(order.internalNotes || []).length === 0 ? (
              ...
              <div className="space-y-2">
                {[...(order.internalNotes || [])].reverse().map((note) => (
                  <div key={note.id} className="flex items-start justify-between gap-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800/50 rounded-lg p-3">
                    <div>
                    ...
                    <button onClick={() => removeNote(note.id)} aria-label="Eliminar nota" className="text-gray-300 hover:text-red-500 p-1">
                      <Trash2 className="h-3.5 w-3.5" />
                    </button>
                  </div>
                ))}
        ...
        {/* Signature - Read Only */}
        <div className="card">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-3">Firma del Cliente</h3>
