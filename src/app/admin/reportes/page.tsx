...
export default function ReportesPage() {
  ...
  const completedOrders = periodOrders.filter(o => o.status === "entregado" || o.status === "listo").length;
  ...
  const avgRepairTime = (() => {
    ...
    const totalDays = delivered.reduce((sum, o) => {
      ...
      // Buscar cuando pasó a "listo" o "entregado"
      const completedEvent = o.statusHistory.find(h => h.to === "listo" || h.to === "entregado");
  ...
  const stuckOrders = orders.filter(o => {
    if (["entregado", "cancelado", "listo"].includes(o.status)) return false;
  ...
  // Órdenes completadas por día de la semana
  const dayNames = ["Dom", "Lun", "Mar", "Mié", "Jue", "Vie", "Sáb"];
  const completedByDayOfWeek = (() => {
    const counts = [0, 0, 0, 0, 0, 0, 0]; // Dom=0, Lun=1, ...
    const completedOrders = periodOrders.filter(o =>
      o.status === "entregado" || o.status === "listo"
    );
    completedOrders.forEach(o => {
      // Buscar fecha de completado en statusHistory
      const completedEvent = o.statusHistory?.find(h => h.to === "listo" || h.to === "entregado");
      const completedDate = completedEvent ? new Date(completedEvent.date) : new Date(o.updatedAt);
      counts[completedDate.getDay()]++;
    });
    return counts;
  })();
  const maxCompletedByDay = Math.max(...completedByDayOfWeek, 1);

  // Tiempo promedio por tipo de dispositivo
  const avgTimeByDeviceType = (() => {
    const typeStats: Record<string, { totalDays: number; count: number }> = {};

    const deliveredOrders = orders.filter(o =>
      o.status === "entregado" && o.statusHistory && o.statusHistory.length > 0
    );

    deliveredOrders.forEach(o => {
      const deviceType = o.deviceType || "Otro";
      if (!typeStats[deviceType]) {
        typeStats[deviceType] = { totalDays: 0, count: 0 };
      }
      ...
      typeStats[deviceType].totalDays += days;
      typeStats[deviceType].count++;
    });
  ...
  const maxAvgTime = Math.max(...avgTimeByDeviceType.map(t => t.avgDays), 1);
  ...
  const monthlyComparison = (() => {
    ...
    const thisMonthCompleted = thisMonthOrders.filter(o => o.status === "entregado" || o.status === "listo").length;
    const lastMonthCompleted = lastMonthOrders.filter(o => o.status === "entregado" || o.status === "listo").length;

    const thisMonthRevenue = thisMonthOrders.reduce((sum, o) => sum + (o.estimatedCost || 0), 0);
    const lastMonthRevenue = lastMonthOrders.reduce((sum, o) => sum + (o.estimatedCost || 0), 0);
  ...
  // Órdenes completadas por semana (últimas 4 semanas)
  const completedByWeek = (() => {
    ...
    for (let i = 3; i >= 0; i--) {
      ...
      const count = orders.filter(o => {
        if (o.status !== "entregado" && o.status !== "listo") return false;
        const completedEvent = o.statusHistory?.find(h => h.to === "listo" || h.to === "entregado");
        const completedDate = completedEvent ? new Date(completedEvent.date) : new Date(o.updatedAt);
  ...
  const maxWeeklyCompleted = Math.max(...completedByWeek.map(w => w.count), 1)
    ...
    <div className="space-y-8 pb-20">
        ...
        <div className="card border-l-4 border-l-green-500">
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{formatMoneyShort(totalRevenue, currency)}</h3>
            </div>
        ...
        <div className="card border-l-4 border-l-blue-500">
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{periodOrders.length}</h3>
            </div>
        ...
        <div className="card border-l-4 border-l-purple-500">
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{Math.round(conversionRate)}%</h3>
            </div>
        ...
        <div className="card border-l-4 border-l-orange-500">
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{avgRepairTime} <span className="text-sm font-medium text-gray-500">días</span></h3>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="card bg-gray-50 border-0">
              <h4 className="font-semibold text-gray-900 dark:text-white mb-4">Desglose Financiero</h4>
              <div className="space-y-3">
                ...
                <div className="border-t border-gray-200 pt-2 flex justify-between text-base font-bold text-gray-900 dark:text-white">
            ...
            {/* Efficiency visual */}
            <div className="card bg-gray-50 border-0">
              <h4 className="font-semibold text-gray-900 dark:text-white mb-4">Eficiencia Operativa</h4>
      ...
      <div className="border-t border-gray-200 pt-8 mt-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          {/* Gráfica: Órdenes Completadas por Día de la Semana */}
          <div className="card">
            <h4 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
              <Calendar className="h-4 w-4 text-gray-500" />
              Órdenes Completadas por Día de Semana
            </h4>
            <div className="flex items-end justify-between h-40 gap-2 px-2">
              {completedByDayOfWeek.map((count, idx) => (
                <div key={idx} className="flex flex-col items-center flex-1">
                  ...
                  <div
                    ...
                    style={{
                      height: `${(count / maxCompletedByDay) * 100}%`,
                      minHeight: count > 0 ? '8px' : '2px',
                      backgroundColor: count === 0 ? '#e5e7eb' : undefined
                    }}
                  />
                  <span className="text-xs text-gray-500 mt-2 font-medium">{dayNames[idx]}</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-gray-400 mt-4 text-center">
              Distribución de reparaciones completadas según el día de la semana
            </p>
          </div>

          {/* Gráfica: Órdenes Completadas por Semana */}
          <div className="card">
            <h4 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
              <TrendingUp className="h-4 w-4 text-gray-500" />
              Órdenes Completadas - Últimas 4 Semanas
            </h4>
            <div className="flex items-end justify-between h-40 gap-4 px-4">
              {completedByWeek.map((week, idx) => (
                <div key={idx} className="flex flex-col items-center flex-1">
                  ...
                  <div
                    ...
                    style={{
                      height: `${(week.count / maxWeeklyCompleted) * 100}%`,
                      minHeight: week.count > 0 ? '8px' : '2px',
                      backgroundColor: week.count === 0 ? '#e5e7eb' : undefined
                    }}
                  />
                  <span className="text-xs text-gray-500 mt-2 font-medium">Sem {week.label}</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-gray-400 mt-4 text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="card">
            ...
            {avgTimeByDeviceType.length === 0 ? (
              <div className="text-center py-8 text-gray-400 text-sm">
                No hay datos suficientes de reparaciones completadas
              </div>
            ) : (
              <div className="space-y-3">
                {avgTimeByDeviceType.map((item, idx) => (
                  <div key={idx}>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-gray-700 font-medium">{item.type}</span>
                      <span className="text-gray-500">
                        {item.avgDays} días <span className="text-xs text-gray-400">({item.count} órdenes)</span>
                      </span>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-2.5">
                      <div
                        ...
                        style={{
                          width: `${(item.avgDays / maxAvgTime) * 100}%`,
                          backgroundColor: item.avgDays <= 2 ? '#22c55e' : item.avgDays <= 5 ? '#f59e0b' : '#ef4444'
                        }}
                      />
                    </div>
                  </div>
            ...
            <p className="text-xs text-gray-400 mt-4">
              <span className="inline-block w-2 h-2 rounded-full bg-green-500 mr-1"></span> 0-2 días
              <span className="inline-block w-2 h-2 rounded-full bg-amber-500 ml-3 mr-1"></span> 3-5 días
              <span className="inline-block w-2 h-2 rounded-full bg-red-500 ml-3 mr-1"></span> +5 días
            </p>
          </div>
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-xl font-bold text-gray-900 dark:text-white">{monthlyComparison.thisMonth.orders}</span>
                    <span className="text-xs text-gray-400">vs {monthlyComparison.lastMonth.orders} mes anterior</span>
                  </div>
                </div>
                <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${
                  monthlyComparison.changes.orders >= 0
                    ? 'bg-green-100 text-green-700'
                    : 'bg-red-100 text-red-700'
                }`}>
                  {monthlyComparison.changes.orders >= 0 ? '↑' : '↓'} {Math.abs(monthlyComparison.changes.orders)}%
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  <p className="text-xs text-gray-500 uppercase font-medium">Completadas</p>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-xl font-bold text-gray-900 dark:text-white">{monthlyComparison.thisMonth.completed}</span>
                    <span className="text-xs text-gray-400">vs {monthlyComparison.lastMonth.completed} mes anterior</span>
                  </div>
                </div>
                <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${
                  monthlyComparison.changes.completed >= 0
                    ? 'bg-green-100 text-green-700'
                    : 'bg-red-100 text-red-700'
                }`}>
                  {monthlyComparison.changes.completed >= 0 ? '↑' : '↓'} {Math.abs(monthlyComparison.changes.completed)}%
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-xl font-bold text-gray-900 dark:text-white">{formatMoneyShort(monthlyComparison.thisMonth.revenue, currency)}</span>
                    <span className="text-xs text-gray-400">vs {formatMoneyShort(monthlyComparison.lastMonth.revenue, currency)}</span>
                  </div>
                </div>
                <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${
                  monthlyComparison.changes.revenue >= 0
                    ? 'bg-green-100 text-green-700'
                    : 'bg-red-100 text-red-700'
                }`}>
                  {monthlyComparison.changes.revenue >= 0 ? '↑' : '↓'} {Math.abs(monthlyComparison.changes.revenue)}%
                </div>
              </div>
            </div>
          </div>
