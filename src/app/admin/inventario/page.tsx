...
import { Package, Plus, Pencil, Trash2, AlertTriangle, Search, X, Check, ArrowUpDown, Filter, Save, Upload, Download, FileSpreadsheet } from "lucide-react";
...
export default function InventarioPage() {
  ...
  const [importPreview, setImportPreview] = useState<{name: string; cost: number; stock: number}[]>([]);
  ...
  const handleExportExcel = () => {
    const exportData = parts.map(part => ({
      "Nombre": part.name,
      "Costo Unitario": part.cost,
      "Stock": part.stock,
      "Valor Total": part.cost * part.stock,
      "Veces Usada": part.timesUsed || 0,
      "Fecha Creación": new Date(part.createdAt).toLocaleDateString(),
      "Última Actualización": new Date(part.updatedAt).toLocaleDateString(),
    }));
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Map and validate data - flexible column name matching
        const preview = jsonData.map((row: unknown, index: number) => {
          const r = row as Record<string, unknown>;

          // Try different possible column names
          const name = r["Nombre"] || r["nombre"] || r["Name"] || r["name"] || r["NOMBRE"] || "";
          const cost = r["Costo Unitario"] || r["costo"] || r["Costo"] || r["cost"] || r["Cost"] || r["COSTO"] || 0;
          const stock = r["Stock"] || r["stock"] || r["STOCK"] || r["Cantidad"] || r["cantidad"] || 0;

          if (!name) {
            throw new Error(`Fila ${index + 2}: Falta el nombre de la pieza`);
          }
  ...
  const handleImport = async () => {
    ...
    if (errors > 0) {
      alert(`Importación completada: ${imported} piezas agregadas, ${errors} errores`);
    } else {
      alert(`${imported} piezas importadas exitosamente`);
    }
  };
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      { "Nombre": "Batería iPhone 12", "Costo Unitario": 25, "Stock": 10 },
      { "Nombre": "Pantalla Samsung A52", "Costo Unitario": 45, "Stock": 5 },
    ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card !p-4 border-l-4 border-l-blue-500">
          ...
          <div className="mt-1 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-gray-900 dark:text-white">{formatMoneyShort(totalValue, currency)}</span>
          </div>
        </div>
      ...
      {/* Table */}
      <div className="card overflow-hidden !p-0 shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                <th
                  className="px-6 py-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group"
                ...
                <th
                  className="px-6 py-3 text-right cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group"
                ...
                <th
                  className="px-6 py-3 text-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group"
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700 bg-white dark:bg-gray-800">
              {filtered.length === 0 ? (
                ...
                filtered.map((part) => {
                  ...
                  return (
                    <tr key={part.id} className="hover:bg-gray-50/80 dark:hover:bg-gray-700/50 transition-colors group">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full p-6 transform transition-all scale-100">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Nombre de la pieza</label>
                <input
                  type="text"
                  value={formName}
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Costo Unitario</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Stock Inicial</label>
                  <input
                    type="number"
                    value={formStock || ""}
              ...
              {formCost > 0 && formStock > 0 && (
                <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg flex justify-between items-center text-sm">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white">Importar Inventario</h3>
                  <p className="text-sm text-gray-500">{importPreview.length} piezas para importar</p>
                </div>
              </div>
              <button
                onClick={() => {
            ...
            <div className="flex-1 overflow-auto p-6">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-800/50 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    ...
                    <th className="px-4 py-2 text-right rounded-r-lg">Stock</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {importPreview.slice(0, 50).map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-right text-gray-600">{formatMoneyShort(item.cost, currency)}</td>
              ...
              {importPreview.length > 50 && (
                <p className="text-center text-sm text-gray-500 mt-4">
                  ... y {importPreview.length - 50} piezas más
                </p>
              )}
            </div>

            <div className="flex justify-between items-center p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 rounded-b-xl">
              <button
                ...
                >
                  {importing ? (
                    ...
                    <>
                      ...
                      Importar {importPreview.length} piezas
                    </>
                  )}
                </button>
              </div>
