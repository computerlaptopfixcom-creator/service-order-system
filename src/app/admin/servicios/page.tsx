    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="card !p-4 border-l-4 border-l-emerald-500 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{formatMoneyShort(totalRevenue, currency)}</p>
          </div>
        ...
        <div className="card !p-4 border-l-4 border-l-purple-500 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{withParts.length}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className={`card sticky top-6 transition-all ${editingId ? "ring-2 ring-primary-500 shadow-lg" : ""}`}>
            ...
            <div className="space-y-4">
                  ...
                  {categories.map(cat => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs rounded-full border transition-colors ${newCategory === cat.name
                        ? "bg-primary-50 dark:bg-primary-900/30 border-primary-200 dark:border-primary-700 text-primary-700 dark:text-primary-400 font-medium"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Precio al Cliente ({getCurrency(currency).symbol})</label>
                <input
                  type="number"
                  value={newPrice || ""}
              ...
              <button
                ...
                className={`w-full py-2.5 flex items-center justify-center gap-2 mt-2 rounded-lg font-medium text-white transition-colors ${editingId ? "bg-primary-600 hover:bg-primary-700" : "bg-gray-900 dark:bg-gray-700 hover:bg-gray-800 dark:hover:bg-gray-600"
                  }`}
              >
                {isSubmitting ? (
        ...
        <div className="lg:col-span-2 space-y-4">
          {/* Filters */}
          <div className="bg-white dark:bg-gray-800 p-2 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row gap-2">
            ...
            <div className="h-8 w-px bg-gray-200 dark:bg-gray-700 hidden sm:block self-center" />
            <div className="flex overflow-x-auto pb-1 sm:pb-0 gap-1 no-scrollbar">
              <button
                onClick={() => setSelectedCategory("Todas")}
                className={`px-3 py-1.5 text-sm rounded-lg whitespace-nowrap transition-colors ${selectedCategory === "Todas" ? "bg-gray-900 dark:bg-white text-white dark:text-gray-900" : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                  }`}
              >
                Todas
              </button>
              {categories.map(cat => (
                <button
                  ...
                  className={`px-3 py-1.5 text-sm rounded-lg whitespace-nowrap transition-colors ${selectedCategory === cat.name ? "bg-gray-900 dark:bg-white text-white dark:text-gray-900" : "text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                    }`}
                >
                  {cat.name}
                </button>
          ...
          <div className="space-y-3">
            {filteredServices.length === 0 ? (
              <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-dashed border-gray-300 dark:border-gray-600">
                ...
                <h3 className="text-gray-900 dark:text-white font-medium">No se encontraron servicios</h3>
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300`}>
                          {svc.category || "General"}
                        </span>
                        <h4 className="font-bold text-gray-900 dark:text-white truncate">{svc.name}</h4>
                      </div>
                      ...
                      {svc.linkedPartName && (
                        <div className="flex items-center gap-1.5 text-xs bg-gray-50 dark:bg-gray-700/50 w-fit px-2 py-1 rounded border border-gray-200 dark:border-gray-600">
                    ...
                    <div className="text-right shrink-0 flex flex-col items-end justify-between h-full min-h-[60px]">
                      <span className="text-lg font-bold text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700/50 px-3 py-1 rounded-lg border border-gray-100 dark:border-gray-600">
                        {formatMoneyShort(svc.basePrice, currency)}
                      </span>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => handleEdit(svc)}
                          className="text-gray-400 hover:text-blue-600 p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors"
                        ...
                        <button
                          ...
                          className="text-gray-400 hover:text-red-600 p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"
      ...
      {showCatModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50 dark:bg-gray-800/80">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {categories.length === 0 ? (
                  ...
                  categories.map(cat => (
                    <div key={cat.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg group">
