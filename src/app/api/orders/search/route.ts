...
import { getOrderByNumber, searchOrdersByPhone, searchOrdersBySerial } from "@/lib/storage";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === "serial") {
      const orders = await searchOrdersBySerial(query);
      if (orders.length === 0) {
        return NextResponse.json({ error: "No se encontraron antecedentes para este serial" }, { status: 404 });
      }
      return NextResponse.json(orders.map(publicOrderData));
    }
